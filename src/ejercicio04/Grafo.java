...
public class Grafo<T> {
    ...
    public Boolean hayCaminoIterativo(Vertice<T> v1,Vertice<T> v2) {
    	List<Vertice<T>> adyacentes = getAdyacentes(v1);
    	for(Vertice<T> vAdy : adyacentes) {
    		if(vAdy.equals(v2)) {
    			return true;
    ...
    public boolean existeCaminoLimpio(Vertice<T> v1,Vertice<T> v2){
        if(esAdyacente(v1, v2)) return true;
        else return existeCaminoLimpio(v1, v2,new HashSet<Vertice<T>>());
    }

    private boolean existeCaminoLimpio(Vertice<T> v1,Vertice<T> v2,Set<Vertice<T>> visitados){
    	if(esAdyacente(v1, v2)) return true;
        List<Vertice<T>> adyacentes = this.getAdyacentes(v1);
        for(Vertice<T> v : adyacentes){
            if(!visitados.contains(v)) {
                visitados.add(v);
    // Al codigo de existen caminos solo le agregue la condicion (this.gradoSalida(v)<=1) 
    // para asegurarme que los nodos interiores no tengan grado de salida mayor a 1.             
                if((this.gradoSalida(v)<=1) && existeCaminoLimpio(v, v2,visitados))return true;
            }
        }
        return false;
    }
